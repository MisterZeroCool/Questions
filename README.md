...
Если активность завершается, `onDestroy()` – это последний метод жизненного цикла, который вызывает `<b>Activity`. Если `onDestroy()` вызывается в результате изменения конфигурации, система немедленно создает новый экземпляр активности вызывает `onCreate()` в новом экземпляре и новой конфигурации.
